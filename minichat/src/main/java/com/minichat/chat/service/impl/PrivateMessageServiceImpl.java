...
import com.minichat.common.core.exception.ChatException;
...
public class PrivateMessageServiceImpl extends AbstractMessageService implements PrivateMessageService {
    ...
    @Override
    public void sendPrivateMessage(PrivateMessageDTO privateMessageDTO, Principal principal) {
        Long realSenderId;
        try {
            realSenderId = Long.parseLong(principal.getName());
        } catch (NumberFormatException e) {
            throw new ChatException(ErrorCode.BAD_REQUEST, "用户身份解析失败");
        }
        ...
        if (receiver == null) {
            throw new ChatException(ErrorCode.BAD_REQUEST, "接收方不存在");
        }
        ...
        if (senderFriendCount == 0 || receiverFriendCount == 0) {
            log.warn("用户 {} 尝试向已删除的好友 {} 发送消息（发送方好友计数: {}, 接收方好友计数: {}）",
                    ...
                    senderFriendCount,
                    receiverFriendCount);
            throw new ChatException(ErrorCode.NOT_FRIEND_CANNOT_CHAT, "发送失败,请先添加对方为好友");
        }
    ...
    public void markMessagesAsRead(Long currentUserId, Long receiverId) {
        if (receiverId == null || receiverId <= 0) {
            throw new ChatException(ErrorCode.BAD_REQUEST, "聊天对象ID不能为空");
        }
