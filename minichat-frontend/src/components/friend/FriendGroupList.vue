    ...
    <el-scrollbar v-else>
      <div v-for="group in store.groups" :key="group.groupName" class="group-item">
        ...
        <div v-show="group.expanded" class="group-content">
           <div v-if="!group.items || group.items.length === 0" class="empty-group">
             暂无好友
           </div>
           <div v-for="item in group.items" :key="item.id" class="list-item" :class="{active: selectedId === item.id}" @click="onSelect(item)">
            <div class="avatar-wrap">
              <el-avatar :size="36" :src="item.avatar">{{ getInitial(item.remark || item.nickname || item.username) }}</el-avatar>
...
<style scoped>
.friend-group-list { height: 100%; display: flex; flex-direction: column; background: var(--el-bg-color); }
.loading { padding: 20px; }
.group-header {
  display: flex; align-items: center; padding: 10px 16px; cursor: pointer;
  user-select: none; color: var(--el-text-color-regular); font-size: 14px;
}
.group-header:hover { background: var(--el-fill-color-light); }
.arrow { margin-right: 6px; transition: transform 0.2s; }
...
.status-dot {
  ...
  background: #ccc; border: 2px solid var(--el-bg-color); border-radius: 50%;
}
.status-dot.online { background: var(--el-color-success); }

.info { flex: 1; overflow: hidden; }
.name { font-size: 14px; color: var(--el-text-color-primary); margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sub { font-size: 12px; color: var(--el-text-color-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.empty-group { padding: 10px 20px; color: var(--el-text-color-secondary); font-size: 12px; }
</style>
