  ...
  <el-container class="chat-window">
    <el-header class="chat-header">
      ...
      <div class="right">
        <el-dropdown trigger="click" @command="handleCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item v-if="friend?.type === 1" command="members">
                群组成员
              </el-dropdown-item>
    ...
    <el-main class="chat-body">
      <div class="messages" ref="listRef" @scroll="onScroll">
        ...
        <div v-for="m in messages" :key="m.timestamp + '_' + (m.id||'')" :id="'msg-' + m.id" :class="['msg', (m.type === 4 || m.type === 5) ? 'system-row' : (m.fromId==='self' ? 'self' : 'other')]">
          ...
          <template v-else-if="m.fromId !== 'self'">
            <el-avatar 
              :size="36" 
              :src="m.senderAvatar || friend?.avatar" 
              class="msg-avatar"
            >
              {{ (m.senderNickname || friend?.name || friend?.remark || friend?.nickname || friend?.username || '').slice(0,1).toUpperCase() }}
            </el-avatar>
            <div class="msg-content">
              <!-- 群聊消息显示发送者昵称 -->
              <div v-if="friend?.type === 1 && m.senderNickname" class="sender-name">
                <span>{{ m.senderNickname }}</span>
                <el-tag v-if="memberRoleMap[m.fromId] === 2" size="small" type="warning" effect="dark" class="role-tag">群主</el-tag>
                <el-tag v-else-if="memberRoleMap[m.fromId] === 1" size="small" type="success" effect="dark" class="role-tag">管理员</el-tag>
              </div>
              <div class="bubble">
                ...
                <div v-else-if="m.type===3" class="file-message">
                  <div class="file-info">
                    ...
                    <a v-if="m.fileUrl" :href="m.fileUrl" target="_blank" download class="file-name">{{ m.fileName || '文件' }}</a>
      ...
      <div v-loading="loadingMembers" class="member-list-container">
        ...
        <el-scrollbar height="400px">
          <div v-for="member in memberList" :key="member.userId" class="member-item">
            <el-avatar :src="member.avatar" :size="40">{{ (member.nickname || member.nicknameInGroup || '').slice(0,1).toUpperCase() }}</el-avatar>
             ...
             <div v-for="friend in filteredFriends" :key="friend.id" class="friend-item">
               <el-checkbox :label="friend.id">
                 <div class="friend-info">
                   <el-avatar :size="32" :src="friend.avatar">{{ (friend.remark || friend.nickname || friend.username || '').slice(0,1).toUpperCase() }}</el-avatar>
           ...
           <el-radio-group v-model="removeList" style="width: 100%; display: block;">
             <div v-for="member in filteredMembersToRemove" :key="member.userId" class="friend-item">
               <el-radio :label="member.userId" style="width: 100%; display: flex; align-items: center;">
                 <div class="friend-info">
                   <el-avatar :size="32" :src="member.avatar">{{ (member.nicknameInGroup || member.nickname || '').slice(0,1).toUpperCase() }}</el-avatar>
                   <div class="member-name">
                      <span class="name">{{ member.nicknameInGroup || member.nickname }}</span>
                      <el-tag v-if="member.role === 1" size="small" type="success" effect="dark" style="margin-left: 5px;">管理员</el-tag>
                   </div>
                 </div>
               </el-radio>
             </div>
        ...
        <div class="search-results" v-loading="searching">
           <div v-for="item in searchResults" :key="item.id" class="result-item" @click="jumpToMessage(item)">
              <div class="result-header">
                  <span class="name">{{ item.senderId === userStore.userInfo?.id ? '我' : (item.senderNickName || item.senderNickname || 'Unknown') }}</span>
...
import { Close, WarningFilled, MoreFilled, Search, Plus, Delete, SwitchButton, UserFilled, Avatar, RefreshLeft } from '@element-plus/icons-vue'
...
const text = ref('')
const initial = computed(() => (props.friend?.remark || props.friend?.nickname || props.friend?.username || '').slice(0,1).toUpperCase())
const friendInitial = computed(() => (props.friend?.remark || props.friend?.nickname || props.friend?.username || '').slice(0,1).toUpperCase())
...
const filteredMembersToRemove = computed(() => {
  ...
  let candidates = memberList.value.filter(m => {
    // 排除自己
  ...
  return candidates.filter(m => 
    (m.nicknameInGroup || '').toLowerCase().includes(k) ||
...
const handleMemberAction = async (command, member) => {
  ...
  if (command === 'setAdmin') {
    ...
    try {
      await ElMessageBox.confirm(`确定要将群主转让给 "${member.nicknameInGroup || member.nickname}" 吗？转让后您将成为普通成员。`, '警告', {
...
.upload-progress { padding: 4px 16px 8px; background-color: var(--el-bg-color-page); }
...
.msg-content { display: flex; flex-direction: column; flex: 1; min-width: 0; max-width: 100%; }
.msg.self .msg-content { align-items: flex-end; }
.msg.other .msg-content { align-items: flex-start; }
.sender-name { font-size: 12px; color: var(--el-text-color-secondary); margin-bottom: 4px; display: flex; align-items: center; gap: 4px; }
...
.bubble { background: var(--el-bg-color-overlay); border: 1px solid var(--el-border-color-light); border-radius: 8px; padding: 8px 12px; line-height: 1.6; word-break: break-word; max-width: 100%; color: var(--el-text-color-primary); }
...
.image-placeholder { padding: 20px; text-align: center; color: var(--el-text-color-secondary); display: none; }
...
.chat-input { border-top: 1px solid var(--el-border-color-light); display: flex; flex-direction: column; gap: 8px; height: 200px; padding: 12px; }
.inner-tools { position: absolute; top: 12px; left: 12px; display: flex; gap: 10px; color: var(--el-text-color-secondary); z-index: 2; }
.input-wrap { position: relative; flex: 1; }
.input-wrap :deep(.el-textarea__inner) { height: 120px; padding-right: 92px; padding-bottom: 12px; padding-top: 36px; border: none; box-shadow: none; outline: none; background: var(--el-bg-color); resize: none; }
.input-wrap :deep(.el-textarea__inner:focus) { border: none; box-shadow: none; outline: none; }
...
.emoji-picker { position: absolute; bottom: 100%; left: 0; width: 320px; height: 280px; background: var(--el-bg-color-overlay); border: 1px solid var(--el-border-color-light); border-radius: 8px; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); z-index: 1000; margin-bottom: 8px; display: flex; flex-direction: column; }
...
.friend-item .el-checkbox { width: 100%; display: flex; align-items: center; }
.friend-item :deep(.el-checkbox__label) { flex: 1; }
...
@keyframes highlight-flash {
  0% { background-color: rgba(64, 158, 255, 0.5); }
